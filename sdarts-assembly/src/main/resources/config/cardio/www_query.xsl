...
<xsl:template match="starts:squery">
	<starts:script>
      ...
      <xsl:text>http://www.cardio.com/cgi-bin/search.cardio.cgi</xsl:text>
		</starts:url>

		<!--xsl:apply-templates select="starts:filter"/-->

    <starts:variable>
      <starts:name>mode</starts:name>
      <starts:value>concept</starts:value>
    </starts:variable>

    <starts:variable>
      <starts:name>sp</starts:name>
      <starts:value>sp</starts:value>
    </starts:variable>
...
<!-- cardio does not support boolean operator -->
<!-- so it does not matter how we map the boolean op here -->
