...
<!-- as being of type "starts_intermediate.dtd"                       -->
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="html/body/table/tr/td[3]/p[contains(string(.), 'Score: ')]"> 
      <xsl:call-template name="makeDoc"/>
    </xsl:for-each>

    <!-- xsltest does not have number function
    <xsl:variable name="numavailable" select="number(substring-after(html/body/table/tr/td[3]/p[2]/text(), 'of '))" /> 
    -->
    <xsl:variable name="numavailable" select="substring-before(substring-after(html/body/table/tr/td[3]/p[2]/text(), 'of '), ' ')"/> 
        <starts:numavailable>
        <xsl:choose>
         ...
         <xsl:otherwise>
          <xsl:value-of select="count(html/body/table/tr/td[3]/p[contains(string(.), 'Score')])"/>
         </xsl:otherwise>
        </xsl:choose>
        </starts:numavailable>
        <xsl:text>&#xa;</xsl:text>
